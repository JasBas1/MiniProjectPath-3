...
model_3 = MLPClassifier(random_state=0, solver='lbfgs')
...
X_train_p_scaled = scaler_p.fit_transform(X_train_poison_reshaped)
X_test_p_scaled = scaler_p.transform(X_test_reshaped)
allnumbers_reshaped = allnumbers_images.reshape(allnumbers_images.shape[0], -1)
allnumbers_p_scaled = scaler_p.transform(allnumbers_reshaped)

model_1_p = GaussianNB()
model_1_p.fit(X_train_p_scaled, y_train)
model1_p_results = model_1_p.predict(X_test_p_scaled)
...
model_2_p.fit(X_train_p_scaled, y_train)
model2_p_results = model_2_p.predict(X_test_p_scaled)
...
model_3_p = MLPClassifier(random_state=0, solver='lbfgs')
model_3_p.fit(X_train_p_scaled, y_train)
model3_p_results = model_3_p.predict(X_test_p_scaled)
...
X_poison_scaled = scaler.fit_transform(X_poison_reshaped)
...
pca.fit(X_poison_scaled)

X_denoised_scaled = pca.inverse_transform(pca.transform(X_poison_scaled))
X_denoised_reshaped = scaler.inverse_transform(X_denoised_scaled)
X_train_denoised = X_denoised_reshaped.reshape(X_train_poison.shape)
...
X_train_d_scaled = scaler_d.fit_transform(X_train_denoised_reshaped)
X_test_d_scaled = scaler_d.transform(X_test_reshaped)
allnumbers_d_scaled = scaler_d.transform(allnumbers_reshaped)

model_1_d = GaussianNB()
model_1_d.fit(X_train_d_scaled, y_train)
model1_d_results = model_1_d.predict(X_test_d_scaled)
...
model2_d_results = model_2_d.predict(X_test_d_scaled)
...
model_3_d = MLPClassifier(random_state=0, solver='lbfgs')
model_3_d.fit(X_train_d_scaled, y_train)
model3_d_results = model_3_d.predict(X_test_d_scaled)
